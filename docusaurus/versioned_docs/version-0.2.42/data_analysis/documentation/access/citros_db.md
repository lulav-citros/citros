...
|**```debug_connect```**|**bool**, default **False**|If **True**, the number of connections and queries which were done by all CitrosDB objects with **debug_connect** set **True**<br />    existing in the current session is recorded.<br />    The information is recorded to the _stat.Stat() object.
|**```log```**|**logging.Logger**, default **None**|Logger to record log. If None, then the new logger is created.
