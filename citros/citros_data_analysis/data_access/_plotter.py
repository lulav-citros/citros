...
class _Plotter:
    ...
    def plot_graph(self, df, x_label, y_label,  ax, legend, title, set_x_label, set_y_label, remove_nan,
        ...
        **kwargs
        ...
        if inf_vals is not None:
            flag = flag & (((abs(df[x_label]) - inf_vals) < 0) & ((abs(df[y_label]) - inf_vals) < 0))
        ...
        for s in sid_list:
            ax.plot(df_copy.loc[s][x_label], df_copy.loc[s][y_label], label = str(s), *args, **kwargs)
    ...
    def plot_3dgraph(self, df, x_label, y_label, z_label, ax, scale, legend, title, set_x_label, set_y_label, set_z_label, 
        ...
        **kwargs
        ...
        if remove_nan:
            ...
            flag = pd.Series(data = [True]*len(df))
        if inf_vals is not None:
            flag = flag & (((abs(df[x_label]) - inf_vals) < 0) & ((abs(df[y_label]) - inf_vals) < 0) & ((abs(df[z_label]) - inf_vals) < 0))
        ...
        sid_list = list(set(df_copy.index))
        for s in sid_list:
            ax.plot(df_copy.loc[s][x_label], df_copy.loc[s][y_label], df_copy.loc[s][z_label], label = str(s), *args, **kwargs)
    ...
    def multiple_y_plot(self, df, x_label, y_labels,  fig, legend, title, set_x_label, set_y_label, remove_nan, inf_vals, *args, **kwargs):
        ...
        **kwargs
        ...
        if len(sid_total_list) <= len(color_main_list):
            color_dict = {sid_total_list[i]: color_main_list[i] for i in range(len(sid_total_list))}
        else:
            color_list =  color_main_list + [tuple(np.random.uniform(0,1, size=3)) for _ in range(len(sid_total_list) - len(color_main_list))]
            color_dict = {sid_total_list[i]: color_list[i] for i in range(len(color_list))}
        ...
        if set_y_label is None:
            ...
            if isinstance(set_y_label, str):
                ...
                if len(set_y_label) != len(y_labels):
                    print(f"the number of labels in 'set_y_label' to set to y-axes do not match the number of graphs.\nProvide {len(y_labels)} labels or set 'set_y_label' = None")
            else:
                set_y_label = y_labels
        ...
        for i, y_label in enumerate(y_labels):
            ...
            if inf_vals is not None:
                flag = flag & (((abs(df_copy[x_label]) - inf_vals) < 0) & ((abs(df_copy[y_label]) - inf_vals) < 0))
            if x_label != y_label:
            ...
            for s in sid_list:
                ax[i].plot(F.loc[s][x_label], F.loc[s][y_label], label = str(s), color = color_dict[s], *args, **kwargs)
        ...
        if legend:
            ...
            fig.legend(list(legend_dict.values()), list(legend_dict.keys()), bbox_to_anchor=(1.0, 0.94),loc ='upper left', title="sid")
        fig.tight_layout()
        if return_flag:
            return fig, ax
        ...
        if set_x_label is None:
            ...
            if isinstance(set_x_label, str):
                ...
                if len(set_x_label) != N:
                    print(f"the number of labels in 'set_x_label' to set to x-axes do not match the number of graphs.\nProvide {N} labels or set 'set_x_label' = None")
            else:
                set_x_label = labels
        ...
        for i in range(N):
            ...
            for j in range(N):
                ...
                if i == j:
                    ...
                    sid_list = list(set(F.index))
                    for s in sid_list:
                        axes[i][j].hist(F[x_label].loc[s], num, histtype = 'step', edgecolor = color_dict[s])
                else:
                    if remove_nan:
                        flag = (df_copy[y_label].notna() & df_copy[x_label].notna())
                    else:
                    ...
                    sid_list = list(set(F.index))
                    for s in sid_list:
                        axes[i][j].plot(F.loc[s][x_label], F.loc[s][y_label], label = str(s), color = color_dict[s], *args, **kwargs)
                    if scale:
    ...
    def plot_sigma_ellipse(self, df, x_label, y_label, ax, n_std, plot_origin, bounding_error, inf_vals, 
        ...
        if inf_vals is not None:
            flag = flag & ((df[x_label].abs() - inf_vals) < 0) & ((df[y_label].abs() - inf_vals) < 0)
        ...
        if len(x) > 1:
            try:
                for n in n_std:
                    _, ellipse_par = analysis.CitrosData()._plot_ellipse(x, y, n, ax, edgecolor=next(color_list))
        ...
        if bounding_error:
            ...
            for i, ellipse_par in enumerate(ellipse_par_list):
                R = analysis.CitrosData()._plot_bounding_error(*ellipse_par, ax)
    ...
    def time_plot(self, var_df, ax, var_name, sids, y_label, title_text, legend, *args, **kwargs):
        ...
        **kwargs
        ...
        for s in sids:
            ax.plot(var_df['Time'].loc[s], var_df[var_name].loc[s], label = s, *args, **kwargs)
